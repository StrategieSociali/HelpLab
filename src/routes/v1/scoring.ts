...
export async function scoringV1Routes(app: FastifyInstance) {
  // GET /api/v1/scoring/config (pubblico, cache 1h)
        ...
        200: {
          ...
          properties: {
            ...
            difficulty_multipliers: {
              type: 'object',
              properties: { low: { type: 'number' }, medium: { type: 'number' }, high: { type: 'number' } }
            },
            target_scaling: { type: 'string' },
            evidence_bonus: {
              type: 'object',
              properties: { judge: { type: 'number' }, auto: { type: 'number' }, user: { type: 'number' } }
            },
  ...
  // POST /api/v1/challenges/preview-scoring (pubblico)
  app.post('/challenges/preview-scoring', {
    ...
    schema: {
      ...
      body: { type: 'object' }, // Zod fa la validazione vera
      response: {
        200: {
          type: 'object',
          properties: {
    ...
    const parsed = previewBodySchema.safeParse((req as any).body)
    if (!parsed.success) {
