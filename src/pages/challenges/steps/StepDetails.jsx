 ...
 * StepDetails.jsx
 * ---------------
 * Step 1 del wizard di creazione challenge.
 * Raccoglie i dati base: titolo, descrizione, tipo impatto,
 * indirizzo, date e visibilità organizzatore.
 *
 * NOTA: impact_type è fondamentale — viene letto da StepTargets
 * per generare automaticamente il payload_schema dei task.
 * Ogni modifica ai valori delle option deve essere allineata
 * con la mappatura in StepTargets.jsx (IMPACT_SCHEMA_MAP).
 */
      ...
      <div className="form-grid">
        ...
        <label>
          ...
          <div className={`hint ${titleLen >= TITLE_MIN ? "ok" : "warn"}`}>
            {titleLen >= TITLE_MIN
              ? "OK"
              : `Minimo ${TITLE_MIN} caratteri (${titleLen}/${TITLE_MIN})`}
          </div>
        </label>
        ...
        <label>
          ...
          <div className={`hint ${descLen >= DESC_MIN ? "ok" : "warn"}`}>
            {descLen >= DESC_MIN
              ? "OK"
              : `Minimo ${DESC_MIN} caratteri (${descLen}/${DESC_MIN})`}
          </div>
        </label>

        {/* TIPO IMPATTO
            ATTENZIONE: i valori delle option devono corrispondere
            alle chiavi di IMPACT_SCHEMA_MAP in StepTargets.jsx.
            Non modificare i value senza aggiornare anche quella mappa. */}
        <label>
          Tipo impatto
          <select
            className="control"
          ...
          {/* Feedback visivo sul tipo scelto */}
          {v.impact_type && (
            <div className="hint ok">
              I task di questa sfida raccoglieranno automaticamente i dati
              rilevanti per questo tipo di impatto.
            </div>
          )}
        </label>
        ...
        <label>
          ...
          <input
            className={`control ${
              addrOk ? "input-valid" : loc.address ? "input-invalid" : ""
            }`}
        ...
        <div className="form-row">
          ...
          <label>
            ...
            <input
              type="date"
              className={`control ${
                datesOk ? "input-valid" : v.deadline ? "input-invalid" : ""
              }`}
              value={v.deadline || ""}
